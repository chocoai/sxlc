...
/*	    accept: {														//只允许选择图片文件
	...
	uploader.on( 'fileQueued', function( file ) {
	    var $li = $(
	    	...
	    	$del = $del = $('<div class="file-del"><span>X</span></div>'),
	        $img = $li.find('img');
	    ...
	    $("#fileList .file-item").each(function(){
			$(this).mouseenter(function(){
				$(this).parent().find(".file-del").show("fast");
			}).mouseleave(function(){
				$(this).parent().find(".file-del").hide("fast");
			});
	    });
		...
		function removeFile(file) {
			...
			$li.off().find('.file-del').off().end().remove();
		}
        ...
        thumbnailWidth = 25 * ratio,
        thumbnailHeight = 25 * ratio,
