...
function stopOrStart(statu,id){
	...
	layer.confirm(ms, {
		  ...
		  //按钮【按钮一】的回调
		$.ajax({
			url : appPath+"/mallBanner/ofMallBanner.do",
				data:{
					id:id,
					statu:statu
				},
...
function upOrDown(id,statu){
		...
		layer.confirm(ms, {
			  ...
			  //按钮【按钮一】的回调
			$.ajax({
				url : appPath+"/mallBanner/moveMallBanner.do",
					data:{
						flag:statu,
						id:id
					},
...
function deleteBanner(){
		...
		layer.confirm(ms, {
			  ...
			  //按钮【按钮一】的回调
			$.ajax({
				url : appPath+"/mallBanner/deleteMallBanner.do",
					data:{
						id:id
					},
					type : "post",
...
function addOrUpdate(type){
	...
	if(type==1){
		...
		var data = $('#table_id').DataTable().rows('.selected').data();
		if(data.length<1){
			layer.alert("请选择要修改的积分商城轮播图！",{icon:0});
			return;
		}
...
function addOrModify(type){
		...
		data.id=bannerId;
		var Posturl = appPath+"/mallBanner/";
		if(type ==1){//添加
			Posturl +="saveMallBanner";
		}else{
			Posturl +="updateMallBanner";
		}
...
function viewPic(btn){
    ...
    if(data.url !=""){
    	$("#picView").attr("src",$("#hostPath").val()+data.url);
    	$(".hideHtml").hide();
    }else{
    	$(".hideHtml").show();
    	$("#picView").attr("src","");
...
//				aaSorting : [ [ 16, "desc" ] ],// 默认第几个排序
		        ...
		        ajax: {  
		        	"url": appPath+"/mallBanner/showMallBannerList.do",   
		            "dataSrc": "results",   
		            "data": function ( d ) {
