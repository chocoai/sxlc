...
public class GuaranteeInfoServiceImpl implements
 GuaranteeInfoService {
	...
	public int handleGuaranteeInfo(Map<String, Object> map,InsertAdminLogEntity entity,String[] sIpInfo) {
		int type = IntegerAndString.StringToInt(map.get("types").toString(), -1);
		...
		// TODO Auto-generated method stub return null;result 0:成功 -1：担保机构名称存在 -2：营业执照号已存在
		handleGuaranteeInfoDaoImpl.handleGuaranteeInfo(map);
		int result=IntegerAndString.StringToInt(map.get("result").toString(),-1);
		if(result == 0 && type==0){
	...
	public int insertManagementCertificate(
			List<ManagementCertificateEntity> entities,InsertAdminLogEntity entity,String[] sIpInfo) {
	...
	public List<InstitutionsRecordsEntity> InstitutionsToRaiseCashRecords(
		...
		// TODO Auto-generated method stub return null;
		return selectGuaranteeInfoDaoImpl.InstitutionsToRaiseCashRecords(pageEntity);
	}
}
