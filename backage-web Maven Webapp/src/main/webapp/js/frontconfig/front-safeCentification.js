...
function stopOrStart(id,statu){
	...
	layer.confirm(ms, {
		  ...
		  //按钮【按钮一】的回调
		$.ajax({
			url : appPath+"/SafetyCertification/ofSafetyCertification.do",
				data:{
					id:id,
					statu:statu
				},
...
function addOrUpdate(type){
	...
	if(type==1){
		...
		if(data.length<1){
			layer.alert("请选择要修改的底部网站认证管理！",{icon:0});
			return;
		}
		$("#bannerId").val(data[0].id);	
		$("#title").val(data[0].title);	
		$("#url").val(data[0].url);	
		$("#imgPath").val(data[0].logo);
		appendImg(data[0].logo,$("#fileList"));
	}
...
function addOrModify(type){
		...
		data.id=bannerId;
		var Posturl = appPath+"/SafetyCertification/";
		if(type ==1){//添加
			Posturl +="saveSafetyCertification";
		}else{
			Posturl +="updateSafetyCertification";
		}
...
function appendImg(imgURL,fileListObj){
	...
	var html = '<div  class="file-item thumbnail">' +
                '<img src="'+imgRealURL+'" style="width:98px;height:37px"/>' +
...
//						aaSorting : [ [ 16, "desc" ] ],// 默认第几个排序
		        ...
		        ajax: {  
		            "url": appPath+"/SafetyCertification/SafetyCertificationList.do",   
		            "dataSrc": "results",   
		            "data": function ( d ) {  
		        ...
		        columns: [  
		                  ...
		                  { title:"网站认证资料标题","data": "title" },
		                  { title:"网站认证资料链接","data": "url" }, 
