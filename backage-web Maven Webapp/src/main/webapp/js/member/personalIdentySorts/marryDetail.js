...
function IdentyDetails(memberId){
	...
	$.ajax( {  
		 url:appPath+"/memberidety/marryIdenty.do",
			data:{
				memberId:memberId,
			},
			type:'post',  
			...
			success:function(data) { 
				if(data!=null){
					...
					if(data.maritalName!=null){
						$("#maritalName").val(data.maritalName);
					}
			...
			success:function(data) { 
				if(data!=null && data!=""){
					if(data == 1){
						...
						window.location.href = appPath+"/web/member/per-marrigeAuthen.jsp?content="+typeId;
					}else if(data == 2){
