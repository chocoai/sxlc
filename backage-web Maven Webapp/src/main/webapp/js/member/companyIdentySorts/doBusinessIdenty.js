		 ...
		 url:appPath+"/companyidety/identyDetail.do",
			...
			success:function(data) { 
				...
				if(data!=null){
					...
					if(data.firmBLN!=null){//营业执照号码
						$("#firmBLN").text(data.firmBLN);
					}
					if(data.firmAddress!=null){//注册地址
						$("#firmAddress").text(data.firmAddress);
					}
			...
			success:function(data) { 
				if(data!=null && data!=""){
					if(data == 1){
						...
						window.location.href = appPath+"/web/member/businessLicenseAuthen.jsp?content="+typeId; 
					}else if(data == 2){
