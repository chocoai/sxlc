		 ...
		 url:appPath+"/companyidety/identyDetail.do",
			...
			success:function(data) { 
				...
				if(data!=null){
					...
					if(data.firmOC!=null){//组织机构代码
						$("#firmOC").text(data.firmOC);
					}
					if(data.sRegDate!=null){//颁发时间
						$("#sRegDate").text(data.sRegDate);	
					}
			...
			success:function(data) { 
				if(data!=null && data!=""){
					if(data == 1){
						...
						window.location.href = appPath+"/web/member/organizationValidation.jsp?content="+typeId; 
					}else if(data == 2){
