						...
						<div class="panel panel-success">
							...
							<div class="panel-body">
								<form id="" class="" action="">
									...
									<span class="con-item conItem1"><span>提交申请时间范围</span><input type="text" id="startDate" class="Record_Date_Min notspecial Wdate dateInput" onFocus="WdatePicker({maxDate: '#F{$dp.$D(\'endDate\')||\'2020-10-01\'}' })"/>-&nbsp;&nbsp;<input type="text" id="endDate" class="Record_Date_Max notspecial Wdate dateInput" onFocus="WdatePicker({minDate: '#F{$dp.$D(\'startDate\')}' ,maxDate:'2020-10-01' })"/></span>
									<span class="con-item conItem"><span>借款人用户名</span><input type="text" class="notspecial Logname"/></span>
									<span class="con-item conItem1"><span>申请失败时间范围</span><input type="text" id="startDate1" class="notspecial Wdate dateInput Check_Date_Min" onFocus="WdatePicker({maxDate: '#F{$dp.$D(\'endDate1\')||\'2020-10-01\'}' })"/>-&nbsp;&nbsp;<input type="text" id="endDate1" class="notspecial Wdate dateInput Check_Date_Max" onFocus="WdatePicker({minDate: '#F{$dp.$D(\'startDate1\')}' ,maxDate:'2020-10-01' })"/></span>
			...
			{	
				ajax: {  
					...
					"data": function ( d ) {
						...
						var Check_Date_Min = encrypt.encrypt($(".Check_Date_Min").val());
						var Check_Date_Max = encrypt.encrypt($(".Check_Date_Max").val());
						...
						d.Logname = Logname;
						d.Check_Date_Min = Check_Date_Min;
						d.Check_Date_Max = Check_Date_Max;
						//设置后台排序参数
						d.ordercolumn = encrypt.encrypt("CHECK_DATE");//排序字段 AMOUNT  RECORD_DATE  CHECK_DATE
						d.orderDsec = encrypt.encrypt(0+"");//1:ASC 0:DESC
						//设置查询页面类型
						d.pgType = encrypt.encrypt("1");
					}  
				},
				columns: [  
				          {title:'',sWidth:"3%", 
				          ...
				          { title:"提交申请时间","data": "recordDate"},  
				          { title:"申请失败时间","data": "checkDate"},  
				          { title:"失败原因","data": "checkRemark"}  
				          ],
