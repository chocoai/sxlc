...
<body class="nav-md">
						...
						<div class="w-content ishow detailAuthen realnameAuthen">
							<fieldset>
								...
								<input type="hidden" id="hostPath" value="${ImgProfix}"/>
									...
									<span><samp>营业执照号码：</samp><label id="firmBLN"></label></span>
									<span><samp>注册地址：</samp><label id="firmAddress"></label></span>
									<span><samp>注册法人：</samp><label id="firmLegal"></label></span>
								</div>
								<div class="introduce">
									<span><samp>注册资金：</samp><label class="moneyFormat" id="regCapital"></label>元</span>
								...
								<div>
									...
									<div id="back"  class="back"  style="display: none">
										<button class="backAuthen" onclick="back('web/member/businessLicenseAuthen.jsp?content=<%=typeId%>')">返回</button>
									</div>
	...
	<script type="text/javascript" src="js/member/companyIdentySorts/doBusinessIdenty.js"></script>
		<script type="text/javascript">
		var identyId = "<%=content%>";
		var typeId =<%=typeId%>;
		var btn = "<%=btn%>"; 
		$("#"+typeId).addClass("active").siblings().removeClass("active");
		IdentyDetails(identyId,typeId);//会员认证详情
	</script>
</body>