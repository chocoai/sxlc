...
function expUpload(type) {
			...
			var uploader = WebUploader.create({
			    ...
			    // 选择文件的按钮。可选。
			    pick: '#file',											//内部根据当前运行是创建，可能是input元素，也可能是flash.
			...
			uploader.on( 'uploadSuccess', function( file,json ) {
				...
				var urlPrefix=url[0];//附件访问url前缀
				uploadUrl=url[1];//附件url路径
				//如果传了input元素id，就将值写进input
				if(uploadUrl != null && uploadUrl != ''){
					if(type==0){
						var text="<div class='authPreviDiv'>"+
		   				"<img class='previewImg'   src='"+urlPrefix+uploadUrl+"'>"+
		   				"<input type='hidden' class='previewHide' value='"+uploadUrl+"' />"+
		   				"</div>";
						$(".authPreviDiv_u").append(text);	
						$(".btn_u").show();
					}else if(type==1){
						$("#previewImg").attr("src",""+urlPrefix+uploadUrl+"");
						$(".previewHide").val(uploadUrl);
	...
	uploader.on( 'uploadSuccess', function( file,json ) {
		...
		if(uploadUrl != null && uploadUrl != ''){
			if(type==0){
				...
				$("#"+id).find(".previewImg").attr("src",""+urlPrefix+uploadUrl+"");
				$("#"+id).find(".previewHide").val(uploadUrl);
				$("#"+id).find(".previewHide").attr("id","1");
			}
		}
...
//多按键。。。多图的上传
function expMoreUpload(className) {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              
	//上传初始化
	var uploader = WebUploader.create({
		auto: true,														//选完文件后，是否自动上传。
	    swf: 'plugs/webuploader/Uploader.swf',					//swf文件路径
	    server: appPath+'/UpdateBsnLicense',	//文件接收服务端。
	    // 选择文件的按钮。可选。
	    pick: className,											//内部根据当前运行是创建，可能是input元素，也可能是flash.
	    fileNumLimit:10,												//个数限制
		//[可选] [默认值：undefined] 验证单个文件大小是否超出限制, 超出则不允许加入队列。
		fileSingleSizeLimit: 1024*512,
	    accept: {														//只允许选择图片文件
