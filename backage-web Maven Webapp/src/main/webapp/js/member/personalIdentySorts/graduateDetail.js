...
function IdentyDetails(memberId){
	...
	$.ajax( {  
		 url:appPath+"/memberidety/graduateIdenty.do",
			data:{
				memberId:memberId,
			},
			type:'post',  
...
function submitIdentyList(){
	...
	$.ajax( {  
		 url:appPath+"/memberidety/submitEducation.do",
			data:{
				memberId:memberId,
				attestId:attestId,
				statu:statu,
			...
			success:function(data) { 
				if(data!=null && data!=""){
					if(data == 1){
						...
						window.location.href = appPath+"/web/member/per-titleAuthen.jsp?content="+typeId; 
					}else if(data == 2){
